...
class Grapple:
    ...
    def create_text_particle(self, txt):
        self.particle_manager.add(
            TextParticle(
                screen=self.screen,
                image=self.font.render(txt, True, (180, 32, 42)),
                pos=self.player.vec,
                vel=(0, -1.5),
                alpha_speed=3,
                lifespan=80,
    ...
    def _grapple_pull(self, event_info):
        distance_travelled = self.sigmoid(
            (pygame.time.get_ticks() - self.grapple_time) / 400
        ) * (self.dist - 20)
    ...
    def _grapple(self, neighboring_tiles, event_info, enemies):
        for neighboring_tile in neighboring_tiles:
            if neighboring_tile.rect.collidepoint(self.grapple_endpoint):
                if self.dist == 0:
                    self.dist = self.grapple_startpoint.distance_to(
                        self.grapple_endpoint
                    )
    ...
    def update(self, event_info, tilemap, enemies):
        ...
        if self.clicked and pygame.time.get_ticks() - self.time_started_hold < 100:
            ...
            self.time_started_hold != 0
            and pygame.time.get_ticks() - self.time_started_hold > 100
        ):
            ...
            if -180 < math.degrees(self.angle) < 0:
                self._grapple(
                    get_neighboring_tiles(
                        tilemap, 2, pixel_to_tile(self.grapple_endpoint)
                    ),
                    event_info,
                    enemies,
                )
    ...
    def draw(self, screen):
        ...
        if self.grappling:
            appl_start, appl_end = self.camera.apply(
                self.grapple_startpoint
            ), self.camera.apply(self.grapple_endpoint)
            vec_start, vec_end = pygame.Vector2(
                appl_start.x, appl_start.y
            ), pygame.Vector2(appl_end.x, appl_end.y)

            pygame.draw.line(screen, (218, 224, 234), vec_start, vec_end, width=2)
...
class Swing:
    ...
    def _grapple(self, neighboring_tiles, event_info, tilemap, enemies):
        for neighboring_tile in neighboring_tiles:
            if neighboring_tile.rect.collidepoint(self.grapple_endpoint):
                ...
                temp_x = (
                    math.cos(math.radians(self.angular_vel))
                    * (self.player.vec.x - self.grapple_endpoint.x)
                    - math.sin(math.radians(self.angular_vel))
                    * (self.player.vec.y - self.grapple_endpoint.y)
                    + self.grapple_endpoint.x
                )
                temp_y = (
                    math.sin(math.radians(self.angular_vel))
                    * (self.player.vec.x - self.grapple_endpoint.x)
                    + math.cos(math.radians(self.angular_vel))
                    * (self.player.vec.y - self.grapple_endpoint.y)
                    + self.grapple_endpoint.y
                )

                self.exit_velocity = pygame.Vector2(
                    temp_x - self.player.vec.x, temp_y - self.player.vec.y
                )

                self.angular_vel += (
                    pygame.Vector2(
                        temp_x - self.grapple_endpoint.x,
                        temp_y - self.grapple_endpoint.y,
                    )
                    .normalize()
                    .x
                    - self.player.vel.x / 12
                ) * 0.18
                self.angular_vel *= 0.995

                for player_neighboring_tile in get_neighboring_tiles(
                    tilemap, 2, self.player.tile_vec
                ):
                    if player_neighboring_tile.rect.colliderect(
                        pygame.Rect(temp_x, temp_y, *self.player.SIZE)
                    ):
    ...
    def update(self, event_info, tilemap, enemies):
        ...
        if self.clicked and pygame.time.get_ticks() - self.time_started_hold < 100:
            ...
            self._grapple(
                get_neighboring_tiles(tilemap, 2, pixel_to_tile(self.grapple_endpoint)),
                event_info,
                tilemap,
                enemies,
            )
    ...
    def draw(self, screen):
        if self.grappling:
            e, f = self.camera.apply(self.grapple_startpoint), self.camera.apply(
                self.grapple_endpoint
            )
